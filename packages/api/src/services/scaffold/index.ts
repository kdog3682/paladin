...
export async function scaffold(file: string, opts: ScaffoldOptions): Promise<ProjectData | null> {
  ...
  if (opts.git.initLocalRepo && !(await git.isRepo())) {
    await git.init()
  }

  if (opts.git.initRemoteRepository && project.isNew) {
    await git.initRemoteRepo(project.name)
  }
